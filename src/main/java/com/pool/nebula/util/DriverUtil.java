...
public final class DriverUtil {
    ...
    public static void loadDriverClass(String driverClass, final String url) {
        if (StringUtils.isBlank(driverClass) && StringUtils.isBlank(url)) {
            throw new ConnectPoolException("loadDriverClass失败，配置缺失");
        }
    ...
    private static String getDriverClassByUrl(final String url) {
        if (StringUtils.isBlank(url)) {
            throw new ConnectPoolException("getDriverClassByUrl失败，配置缺失");
        }
        ...
        throw new ConnectPoolException("Can't auto find match driver class for url: " + url);
    }
